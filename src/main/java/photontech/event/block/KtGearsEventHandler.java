...
public class KtGearsEventHandler {
    ...
    public static void onGearSynchronize(KtEvent.KtGearSynchronizeEvent event) {
        ...
        for (BlockPos searchPos : searchPoses) {
            ...
            if (te instanceof KtGearTile) {
                ...
                fixedPhase -= Math.PI / neighborGear.getTeethNumber();
                ...
                neighborMainKt.referenceState.phase = fixedPhase;
                neighborMainKt.referenceState.frequency = frequencyLevel;
    ...
    private static int getFrequency(int frequencyLevel) {
        return (int) Math.pow(2, frequencyLevel);
    }
    ...
    private static int squareInt(int i) {
        return i * i;
    }
